...
function List() {
    ...
    return (
        <>
            ...
            <form onSubmit={(e) => createNote(e)} className="flex flex-col mt-10 gap-4">
                ...
                notes.map((note, i) => {
                    return (
                        <div className=" bg-slate-100 p-2 rounded-md block"  key={i}>
                            <p className="font-bold text-lg text-left">{note.note} <button  className='border border-black' onClick={() => deleteNote(note.id)}> ❌ </button> <button className='border border-black' id="editar">  Editar✏ </button></p>
                        </div>
                    )
                })
            }
